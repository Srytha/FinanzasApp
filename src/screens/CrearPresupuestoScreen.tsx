...
export default function CrearPresupuestoScreen({ onGuardar, onCancelar }: CrearPresupuestoScreenProps) {
  const [form, setForm] = useState<PresupuestoForm>({ nombre: "", limite: "", fechaInicio: "", fechaFin: "", categoria: "", alerta80: true, alerta100: false, notificaciones: true });
  ...
  return (
    <div style={s.screen}>
      ...
      <input style={s.input} placeholder="Almuerzos" value={form.nombre} onChange={(e) => set("nombre", e.target.value)} />
      <p style={s.label}>Límite de gasto</p>
      <input style={s.input} placeholder="$120.000" type="number" value={form.limite} onChange={(e) => set("limite", e.target.value)} />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
        <button style={s.alertBtn(form.alerta80)} onClick={() => set("alerta80", !form.alerta80)}>80% del límite</button>
        <button style={s.alertBtn(form.alerta100)} onClick={() => set("alerta100", !form.alerta100)}>100% del límite</button>
      </div>
      <p style={{ fontSize: 11, color: "#888", marginBottom: 16 }}>Puedes activar una o ambas alertas.</p>
      <button style={s.btnPrimary} onClick={() => onGuardar(form)}>Crear presupuesto</button>
